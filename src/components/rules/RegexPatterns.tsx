...
export const RegexPatterns = forwardRef<RegexPatternsRef, RegexPatternsProps>(function RegexPatternsComp(props, ref) {
    const { patterns, onAdd, onUpdate, onDelete, onDragStart, onDragOver, onDrop, isDragged } = props;
    const [inputValue, setInputValue] = React.useState('');
    const [editingIndex, setEditingIndex] = React.useState<number | null>(null);
    const [presets, setPresets] = React.useState<Array<{ name: string; regex: string; flags?: string }>>([]);
    ...
    const filtered = React.useMemo(() => {
        ...
        if (!q) return presets;
        return presets.filter(p => p.name.toLowerCase().includes(q) || p.regex.toLowerCase().includes(q));
    }, [inputValue, presets]);
                        ...
                        {editingIndex !== null ? (
                            <>
                                ...
                                <TextButton
                                    size="medium"
                                    variant="outlined"
                                    onClick={() => { setEditingIndex(null); setInputValue(''); setOpen(false); }}
