...
const Workspace: React.FC<WorkspaceProps> = ({ documentPath, onBackToList }) => {
  ...
  useEffect(() => {
    reverseIndexRef.current = reverseIndex;
  }, [reverseIndex]);
  useEffect(() => {
    ...
    const recomputeNow = () => {
      ...
      const clampRectTo = (
        ...
        out: { left: boolean; right: boolean; top: boolean; bottom: boolean };
      } => {
        if (!rect || !container)
          return {
            rect,
    ...
    if (scrollersRef.current.length === 0) {
      scrollersRef.current = Array.from(
        document.querySelectorAll('[data-scroll-listener]'),
      ) as HTMLElement[];
      ...
      try {
        ...
        if (result.extractions.length > 0) {
          ...
          result.extractions.forEach(extraction => {
            ...
            const poFieldId =
              EXTRACTION_FIELD_MAPPING[extraction.extractionFieldId] ||
    ...
    try {
      ...
      if (result.extractions.length > 0) {
        ...
        result.extractions.forEach(extraction => {
          ...
          // Use the centralized mapping to convert extraction field IDs to purchase order field IDs
          const poFieldId =
            EXTRACTION_FIELD_MAPPING[extraction.extractionFieldId] || extraction.extractionFieldId;
      ...
      alert(
        `Failed to rerun extraction: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  };
  ...
  const handleCenterIconClick = useCallback(
    (conn: OverlayConnection, index: number) => {
      ...
      setConnections(prev =>
        prev.filter((c, i) => i !== index && !(c.fieldId === fieldId && c.boxId === boxId)),
      );
  ...
  const handleViewerTransformChange = useCallback(
    (_scale: number, _pos: { x: number; y: number }) => {
      const fn = recomputeRef.current;
      if (fn) {
        requestAnimationFrame(() => fn());
      }
      ...
      <Box sx={WORKSPACE_INNER_SX}>
        ...
        {!showConnectionsDebug && (
          <Box
            ...
            sx={WORKSPACE_SHOW_DEBUG_BUTTON_SX}
          >
            Show Debug
          </Box>
        )}
