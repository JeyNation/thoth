...
import { VIEWER_SVG_HOST_SX, VIEWER_SVG_TRANSFORM_SX } from '../../styles/viewerStyles';
...
export const ViewerPage: React.FC<ViewerPageProps> = ({
  ...
  return (
    <Box key={pageIndex} data-page-index={pageIndex}>
      ...
      >
        <Box
          sx={VIEWER_SVG_TRANSFORM_SX(pageWidth || 'auto', pageHeight || 'auto', scale, isDragging)}
        >
          <div dangerouslySetInnerHTML={{ __html: pageContent }} />
          {showOverlays &&
            pageBoxes.map(boundingBox => {
              const style = getBoundingBoxStyle(boundingBox);
              const isFocusedLinked = !!(
                boundingBox.id && focusedInputLinkedBoxIds.has(boundingBox.id)
              );
              return (
                <Box
